<p class="text-muted"><i class="fa fa-comment" aria-hidden="true"></i>
    <small>
        ...
        @else
            Sem respostas! Escreva uma resposta
        @endif
    </small>
</p>
<hr>
@if($post->getCommentCount() > 2 && (empty($comment_count) || $comment_count < 3))
    <a class="btn btn-link btn-block btn-xs" href="{{ url('/post/'.$post->id) }}"><i class="fa fa-bars" aria-hidden="true"></i> Exibir todos os comentários</a>
@endif